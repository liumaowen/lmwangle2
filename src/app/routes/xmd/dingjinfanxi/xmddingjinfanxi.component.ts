...
@Component({
  selector: 'app-xmddingjinfanxi',
  templateUrl: './xmddingjinfanxi.component.html',
  styleUrls: ['./xmddingjinfanxi.component.scss']
})
export class XmdDingjinfanxiComponent implements OnInit {
  search: any = {};
  gridOptions: GridOptions;

  constructor(public settings: SettingsService, private xmdApi: XmdService, private datepipe: DatePipe) {
    this.gridOptions = {
  ...
  listDetail() {
    // 从服务器获取数据赋值给网格
    this.xmdApi.dingjinfanxi(this.search).then((data) => {
