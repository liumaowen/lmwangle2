...
@Component({
  selector: 'app-xmddingjinshouxi',
  templateUrl: './xmddingjinshouxi.component.html',
  styleUrls: ['./xmddingjinshouxi.component.scss']
})
export class XmddingjinshouxiComponent implements OnInit {
  search: any = {};
  gridOptions: GridOptions;
  @ViewChild('shouxidialog') private shouxidialog: ModalDirective;
  constructor(public settings: SettingsService, private xmdService: XmdService, private datepipe: DatePipe) {
    this.gridOptions = {
    ...
    this.gridOptions.columnDefs = [
      {
        ...
        cellRenderer: (params) => {
          return '<a target="_blank" href="#/xmdqihuo/' + params.data.orderid + '">' + params.data.orderbillno + '</a>';
        }
      },
  ...
  listDetail() {
    ...
    // 从服务器获取数据赋值给网格
    this.xmdService.dingjinshouxi(this.search).then((data) => {
  ...
  calcdingjinshouxi() {
    if (confirm('你确定计算吗？')) {
      this.xmdService.calcdingjinshouxi(this.search).then((data) => {
      });
    }
  }
