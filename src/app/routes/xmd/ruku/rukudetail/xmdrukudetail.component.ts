...
@Component({
  selector: 'app-xmdrukudetail',
  templateUrl: './xmdrukudetail.component.html',
  styleUrls: ['./xmdrukudetail.component.scss']
})
export class XmdrukudetailComponent implements OnInit {
  ...
  companyProduce;
  constructor(private actroute: ActivatedRoute, private xmdrukuapi: XmdrukuService, public settings: SettingsService,private addressparseService: AddressparseService,
        ...
        onCellClicked: (params) => {
          if (params.data.id) {
            sweetalert({
              ...
              this.xmdrukuapi.delRukudet(params.data.id).then((response) => {
    ...
    this.feegridOptions.columnDefs = [
      ...
      },
      {
        cellStyle: { 'text-align': 'center' }, headerName: '操作', field: 'caozuo', minWidth: 40, cellRenderer: (params) => {
          ...
          if (params.data.group) {
            if (confirm('你确定删除吗？')) {
              this.xmdrukuapi.delcaigoufee(params.data.group).then(data => {
                this.toast.pop('success', '删除成功！');
                this.listFeeDetail();
                this.getDetail();
              });
  ...
  ngOnInit() {
    ...
    this.xmdrukuapi.get(this.actroute.params['value']['id']).then(data => {
      this.rukuModel = data;
  ...
  // 获取入库详情表
  getDetail() {
    this.xmdrukuapi.findAll(this.actroute.params['value']['id']).then(data => {
  ...
  // 撤销入库
  delXmdruku(id) {
    this.xmdrukuapi.revoke(id).then((response) => {
      this.toast.pop('success', '完成撤销!');
      this.route.navigateByUrl('xmdruku');
    });
  }
  ...
  addFeeDialog() {
    ...
    const xmdrukudets = this.gridOptions.api.getModel()['rowsToDisplay']; // 获取选中的提货单明细

    let weight: number = 0;
    for (let i = 0; i < xmdrukudets.length; i++) {
      if (xmdrukudets[i].selected && xmdrukudets[i].data.id) {
        console.log(xmdrukudets[i]);
        console.log('weight', weight);
        weight = weight['add'](xmdrukudets[i].data.weight);
        console.log('weight', weight);
        this.detids.push(xmdrukudets[i].data.id);
      }
    }
  ...
  createFee() {
    ...
    this.closefeedialog();
    this.xmdrukuapi.createCaigoufee(this.caigoufee).then(() => {
  ...
  // 查询费用明细
  listFeeDetail() {
    this.xmdrukuapi.listFeeDetail(this.rukuid).then(data => {
  ...
  // 汇总查询
  gethuizongdetail() {
    this.xmdrukuapi.listCollectByrukuid(this.rukuid).then(data => {
 ...
 */
  ...
  deleterukudet() {
    ...
    if (confirm('你确定要删除吗？')) {
      this.xmdrukuapi.deleterukudet(this.rukudetids).then(data => {
        this.toast.pop('success', '删除成功！'); 
        this.getDetail();
        this.listFeeDetail();
...
deleterukudetfee() {
    ...
    if (confirm('你确定要删除吗？')) 
      this.xmdrukuapi.deletefee(this.rukudetfeeids).then(data => {
        this.toast.pop('success', '删除成功！'); 
        this.listFeeDetail();
        this.getDetail();
      });
