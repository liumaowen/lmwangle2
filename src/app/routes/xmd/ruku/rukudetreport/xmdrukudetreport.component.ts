...
@Component({
  selector: 'app-xmdrukudetreport',
  templateUrl: './xmdrukudetreport.component.html',
  styleUrls: ['./xmdrukudetreport.component.scss']
})
export class XmdrukudetreportComponent implements OnInit {
  addData: any = {}; // 上传的参数
  // 查询条件对象
  search = {
  ...
  attrs = [];
  constructor(private xmdrukuapi: XmdrukuService, public settings: SettingsService, private userapi: UserapiService,
    private datepipe: DatePipe, private toast: ToasterService,private xmdcustomerApi: XmdcustomerService) {
    this.gridOptions = {
  ...
  selectGnAction(key) {
    ...
    this.xmdrukuapi.getGoodscodeAttribute({ gnid: this.search['gnid'] }).then(data => {
      data.forEach(element => {
  ...
  xmdrukudet() {
    ...
    this.xmdrukuapi.xmdrukudet(search).then(data => {
  ...
  uploads($event) {
    ...
    if ($event.length !== 0) {
      this.xmdrukuapi.create(addData).then(data => {
        if (data.length) {
          ...
          if (kulingtixing) {
            ...
            if (is) {
              ...
              this.xmdrukuapi.create(addData).then(data1 => {
                sweetalert.close();
              ...
              this.xmdrukuapi.deletefile(addData).then(data1 => {
                sweetalert.close();
              });
            }
          });
  ...
  /**推送信息 */
  sendmsg() {
    this.xmdrukuapi.rukusendmsg(this.addData).then(data => {
      this.xmdrukudet();
  ...
  findWiskind() {
    if (this.companyIsWiskind.length < 1) {
      ...
      this.xmdcustomerApi.findwiskind().then((response) => {
