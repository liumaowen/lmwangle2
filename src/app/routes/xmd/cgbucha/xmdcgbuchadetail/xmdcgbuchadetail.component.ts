...
@Component({
  selector: 'app-xmdcgbuchadetail',
  templateUrl: './xmdcgbuchadetail.component.html',
  styleUrls: ['./xmdcgbuchadetail.component.scss']
})
export class XmdCgbuchadetailComponent implements OnInit {
  ...
  constructor(public settings: SettingsService, private cgbuchaApi: XmdCgbuchaapiService, private route: ActivatedRoute,
  ...
  deletebucha() {
    if (confirm('确认要删除补差单吗？')) {
      this.cgbuchaApi.removeById(this.caigouModel['id']).then(data => {
        this.router.navigate(['xmdcgbucha']);
      });
    }
  }
  importfanli() {
    ...
    this.modalService.config.class = 'modal-all';
    this.cgbsModalRef = this.modalService.show(XmdCgbuchaimportComponent);
