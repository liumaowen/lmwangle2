...
@Component({
  selector: 'app-xmdcgbucha',
  templateUrl: './xmdcgbucha.component.html',
  styleUrls: ['./xmdcgbucha.component.scss']
})
export class XmdCgbuchaComponent implements OnInit {
  ...
  constructor(private userApi: UserapiService, private cgbuchaApi: XmdCgbuchaapiService, private router: Router,
  ...
  createbucha() {
    ...
    this.cgbuchaApi.createcgbucha(this.create).then(data => {
      ...
      this.router.navigate(['xmdcgbucha', data.id]);
    });
  ...
  findWiskind() {
    if (this.companyIsWiskind.length < 1) {
      this.companyIsWiskind.push({ label: '请选择卖方单位', value: '' },{ label: '山东新美达科技材料有限公司', value: 26516 })
