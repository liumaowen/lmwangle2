...
import {XmdCaigoudetimportComponent} from '../import/xmdcaigoudetimport.component';
import {MaGangAddComponent} from '../../../../dnn/shared/magangadd/magangadd.component';

const sweetalert = require('sweetalert');

@Component({
  selector: 'app-xmdcaigoudet',
  templateUrl: './xmdcaigoudet.component.html',
  styleUrls: ['./xmdcaigoudet.component.scss']
})
export class XmdCaigoudetComponent implements OnInit {
  ...
  constructor(private caigouApi: XmdCaigouService, private fb: FormBuilder, private actroute: ActivatedRoute, public settings: SettingsService,
  ...
  addmdm() {
    if (!this.det['price']) {
      this.toast.pop('error', '采购单价不能为空！', '');
      return;
    }
    if (!this.det['caigouweight'] || this.det['caigouweight'] === '0') {
      this.toast.pop('error', '采购量不能为空不能为零！', '');
      return;
    }
    if (this.jiaohuodate) {
      this.det['jiaohuodate'] = this.datepipe.transform(this.jiaohuodate, 'y-MM-dd');
    }
    if (!this.det['jiaohuodate'] || this.det['jiaohuodate'] === null) {
      this.toast.pop('error', '交货日期必填！', '');
      return;
    }
    ...
    if (this.det['zhuanhuo'] === '') {
      this.toast.pop('error', '货权是否转移必选！', '');
      return;
    }
    if (this.det['zhuanhuo'] === true) {
      if (this.det['orgid'] === '') {
        this.toast.pop('error', '转货必须选收货机构！', '');
        return;
      }
      if (this.det['orgid'] === '670') {
        this.toast.pop('error', '转货不允许选择涂镀应用艺术为收货机构！', '');
        return;
      }
    }
    if (this.det['detid']) {
  ...
  importdet() {
    ...
    this.cgbsModalRef = this.bsModalService.show(XmdCaigoudetimportComponent);
  ...
  next() {
    if (this.attrs.length !== this.det['guige'].length) {
      this.toast.pop('error', '请把属性填写完整再提交！', '');
      return;
    }
    this.nextflag = true;
    this.nselect.active = [];
    ...
    this.mdmService.createMaterial(this.goodscode).then(good => {
      ...
      this.mdmService.getchandigongcha(chandigongchaparam).then(chandigongchas => {
        ...
        if (this.editTempParam.detdata) {
          ...
          this.det['zhuanhuo'] = this.editTempParam.detdata['zhuanhuo'] === '转货';
  ...
  openmodifymain() {
    ...
    this.kinds = [{label: '请选择合同性质', value: ''}, {value: '1', label: '期货'}, {value: '2', label: '现货'}, {
      value: '3',
      label: '调货'
    }];
    this.types = [{label: '请选择采购类型', value: ''}, {value: '1', label: '工程单'}, {value: '2', label: '库存销售'}, {
      value: '3',
      label: '市场调货'
    }, {value: '4', label: '维实外采'}, {value: '5', label: '现货'}, {value: '6', label: '期货'}];
