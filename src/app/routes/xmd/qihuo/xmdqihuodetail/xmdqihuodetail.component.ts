...
@Component({
  selector: 'app-xmdqihuodetail',
  templateUrl: './xmdqihuodetail.component.html',
  styleUrls: ['./xmdqihuodetail.component.scss']
})
export class XmdQihuodetailComponent implements OnInit {
  oldyufurate: any = '';
  ...
  constructor(public settings: SettingsService, private xmdqihuoapi: XmdQihuoService, private classifyapi: ClassifyApiService,
    private addressparseService: AddressparseService, private caigouApi: CaigouService, private datepipe: DatePipe,
    private toast: ToasterService, private route: ActivatedRoute, private router: Router,
    ...
    public mdmService: MdmService, private orderApi: OrderapiService, private classifyApi: ClassifyApiService,private xmdorderService: XmdorderService) {
    //aggird实例对象
    this.gridOptions = {
      ...
      getContextMenuItems: (params) => {
        ...
        if (!this.qihuomodel['vuserid']) {
          result.push({
            name: '行复制',
            action: () => {
              this.xmdqihuoapi.copydet({ id: params.node.data.id }).then(() => {
                this.getqihuomodel();
                this.findqihuodet();
              })
            }
        ...
        children: [
          {
            cellStyle: { 'text-align': 'center' }, headerName: '删除', field: '', minWidth: 60, enableRowGroup: true, cellRenderer: data => {
              ...
              if (this.qihuoflag['issubmit']) {
                if (confirm('你确定要删除吗？')) {
                  this.xmdqihuoapi.deldet(data.data.id).then(() => {
        ...
        cellStyle: { 'text-align': 'center' }, headerName: '引入完成', field: '', minWidth: 60, enableRowGroup: true, cellRenderer: data => {
          ...
          if (!params.data.finish && params.data.id && (this.qihuomodel['ordertype'] === 0 || this.qihuomodel['ordertype'] === 1|| this.qihuomodel['ordertype'] === 15) ) {
            sweetalert({
              ...
              this.xmdqihuoapi.finish(params.data.id).then(data => {
    ...
    this.pgridOptions.columnDefs = [
      ...
      },
      {
        ...
        //   if (confirm('你确定要删除吗?')) {
        //     this.xmdqihuoapi.delproduct(params.data.id).then(data => {
        //       this.toast.pop('success', '删除成功！');
        //       this.queryproduct();
        //       this.getqihuomodel();
    ...
    this.bmgridOptions.columnDefs = [
      ...
      { cellStyle: { 'text-align': 'center' }, headerName: '资源号', field: 'grno', minWidth: 60, enableRowGroup: true },
      {
        ...
        onCellClicked: (params) => {
          if(!(this.qihuomodel['ordertype'] === 1 || this.qihuomodel['ordertype'] === 17 || this.qihuomodel['ordertype'] === 19)){
            if (confirm('你确定要删除吗？')) {
              this.xmdqihuoapi.deleteProorderdet(params.data.id).then(data => {
    ...
    this.wuliuOffergridOptions.columnDefs = [
      ...
      },
      {
        cellStyle: { 'text-align': 'center' }, headerName: '操作', field: '', width: 90, enableRowGroup: true, cellRenderer: data => {
          ...
          this.xmdqihuoapi.sccinquirystatus(obj).then(data1 => {
            let inquiry: any = {};
            if (data1.length) {
              inquiry = data1[0];
            }
...
//     }
//     // 获取物流竞价明细表
//     this.xmdqihuoapi.wuliuofferdetail(this.qihuoid).then(data => {
...
//   }
  ...
  modifyinterestfree() {
    ...
    this.xmdqihuoapi.modifyinterestfree(this.interestfreeAndUnitAndCangkuObj.qihuodetid, this.interestfreeAndUnitAndCangkuObj).then(data => {
  ...
  modifyattr(params, type) {
    ...
    this.xmdqihuoapi.modifyqihuodet(params.data.id, { type: type, value: params.newValue }).then(data => {
      this.toast.pop('success', '修改成功');
      this.getqihuomodel();
      this.findqihuodet();
    }, err => {
  ...
  modifyxhlianxiren(params) {
    ...
    this.xmdqihuoapi.editxhlianxiren(obj).then(data => {
      this.toast.pop('success', '修改成功');
    //   this.getMyRole();
    });
  }
  //获取orderdet中的数据
  getorderdet() {
    this.xmdqihuoapi.findOrderdet(this.qihuoid).then(data => {
      if (data.length > 0) {
  ...
  //获取主表的数据
  getqihuomodel() {
    this.xmdqihuoapi.findqihuo(this.qihuoid).then(data => {
  ...
  modify(params) {
    this.xmdqihuoapi.update(this.qihuomodel['id'], params).then(() => {
      if (this.qihuomodel['qihuostatus'] === 8) {
        this.getqihuomodel();
        setTimeout(() => {
  ...
  //查询期货明细
  findqihuodet() {
    this.xmdqihuoapi.findQihuodet(this.qihuoid).then(data => {
  ...
  createproduct() {
    ...
    this.xmdqihuoapi.createProduct(this.productmodel).then(() => {
  ...
  queryproduct() {
    if (this.qihuoflag['fp']) {
      this.xmdqihuoapi.findProduct(this.qihuoid).then(data => {
  ...
  //如果是期货加工则获取基料
  getbasematerial() {
    this.xmdqihuoapi.findBasematerial(this.qihuoid).then(data => {
  ...
  adddingjindialog() {
    ...
    this.xmdorderService.getmoney(moneyquery).then(data => {
  ...
  adddingjin() {
    ...
    this.xmdqihuoapi.adddingjin(model).then(() => {
  ...
  verifydingjin(dingjinid) {
    if (confirm("这个定金确定要审核通过吗？")) {
      ...
      this.xmdqihuoapi.verifydingjin(this.qihuomodel['id'], dingjinid).then(() => {
  ...
  shifang() {
    ...
    this.xmdqihuoapi.shifangdingjin(model).then(() => {
  ...
  querydingjin() {
    this.xmdqihuoapi.finddingjin(this.qihuoid).then(data => {
      data.forEach(element => {
  ...
  submitverify() {
    ...
    if (confirm('你确定提交审核吗？')) {
      this.xmdqihuoapi.submitverify(this.qihuoid, search).then(data => {
  ...
  verifyqihuo() {
    ...
    if (confirm("你确定审核吗？")) {
      this.xmdqihuoapi.verify(this.qihuoid, search).then(data => {
        this.toast.pop('success', '审核成功');
        this.getqihuomodel();
      })
    }
  ...
  removeverify() {
    let search = { version: this.qihuomodel['version'] };
    if (confirm("你确定弃审吗？")) {
      this.xmdqihuoapi.qishen(this.qihuoid, search).then(data => {
  ...
  //通知采购
  noticecaigou() {
    this.xmdqihuoapi.findfujian(this.qihuoid).then(data => {
    this.fujian = data['isfujian'];
    console.log(this.fujian);
    console.log(123);
    if(this.fujian){
      if (confirm('你确定通知采购吗？')) {
        this.xmdqihuoapi.noticeCaigou(this.qihuoid).then(data => {
  ...
  noticerukuapply() {
    ...
    if (confirm('你确定通知入库申请吗？')) {
      this.xmdqihuoapi.noticerukuapply(this.qihuoid).then(data => {
        this.toast.pop('success', '通知成功！');
        this.getqihuomodel();
      });
    }
  ...
  //引入
  imp(ids) {
    this.xmdqihuoapi.impkucun(this.qihuodetid, { kucunids: ids }).then(data => {
  ...
  //打印预览
  print() {
    this.xmdqihuoapi.print(this.qihuoid).then(data => {
      if (!data['flag']) {
  ...
  //生成pdf
  makepdf() {
    this.xmdqihuoapi.makepdf(this.qihuoid,this.changeordermodal).then(data => {
  ...
  getqihuochangeloglist() {
    this.xmdqihuoapi.getqihuochangeloglist(this.qihuoid).then(res => {
      const data = res['list'];
  ...
  finishqihuo(qihuoid) {
    ...
    if (confirm('所有入库已经完成并且订单中货物已经实提，你确定要这么做吗？')) {
      this.xmdqihuoapi.finishqihuo(qihuoid, null).then(data => {
  ...
  modifychukufee() {
    if (!isNaN(this.chukufeetypess['value'])) {
      ...
      this.xmdqihuoapi.getchukufeetype(this.chukufeetypess).then(() => {
        this.getorderdet();
        this.toast.pop('success', '修改成功');
  ...
  addyugufeetodb() {
    ...
    if (confirm("你确定要添加费用吗？")) {
      this.xmdqihuoapi.addfeeinqihuo(this.feemodel).then(data => {
        this.toast.pop('success', '添加成功!');
  ...
  modifysaleprice() {
    ...
    if (confirm("你确定价格修改无误，提交修改吗？")) {
      this.xmdqihuoapi.modifyprice(this.qihuodetid, this.salepricemodel).then(data => {
        this.findqihuodet();
        this.getqihuomodel();
  ...
  delorderfeeindb(orderfeedetid, feeprice) {
    if (confirm('你确定要删除吗？')) {
      this.xmdqihuoapi.delorderfee(orderfeedetid, this.qihuodetid).then(data => {
        this.findyugufee(this.qihuodetid);
  ...
  findyugufee(qihuodetid) {
    this.xmdqihuoapi.findYugufees(qihuodetid).then(data => {
      this.diyfeeHeji = 0;
      this.lines = data;
  ...
  hidewuliuidqd() {
    ...
    this.xmdqihuoapi.addwuliuyuan(this.qihuomodel).then(() => {
    });
    this.getqihuomodel();
    this.hidewuliuid();
  }
  ...
  hesuandingjin() {
    if (confirm('确定后定金开始核算，你确定吗？')) {
      this.xmdqihuoapi.hesuandingjin(this.qihuoid).then(data => {
        this.toast.pop('success', '修改成功！');
        this.querydingjin();
      });
    }
  ...
  upcontract($event) {
    ...
    if ($event.length !== 0) {
      this.xmdqihuoapi.uploadcontract(model).then(data => {
  ...
  lookContract() {
    this.xmdqihuoapi.lookContract(this.qihuoid).then(data => {
      console.log(data);
      if (!data['flag']) {
        this.toast.pop('warning', data['msg']);
      } else {
  ...
  cancelnoticecaigou() {
    if (confirm('你确定要取消通知采购吗？')) {
      this.xmdqihuoapi.cancelnoticecaigou(this.qihuoid).then(data => {
        this.getqihuomodel();
        this.toast.pop('success', '操作成功！');
      });
    }
  ...
  zuofeiorder() {
    ...
    if (confirm('你确定要作废吗？')) {
      this.xmdqihuoapi.zuofeiOrder(this.qihuoid, this.qihuomodel['zuofeireason']).then(data => {
        this.getqihuomodel();
        this.toast.pop('success', '操作成功！');
        this.zuofeiclose();
      });
  ...
  addallocation() {
    ...
    this.xmdqihuoapi.addAllocation(model).then(() => {
      this.toast.pop('success', '添加成功'); 
  ...
  verifyAllocation(allocationid) {
    if (confirm('这个定金确定要审核通过吗？')) {
      this.xmdqihuoapi.verifyAllocation(this.qihuomodel['id'], allocationid).then(() => {
        this.toast.pop('success', '审核成功');
        this.queryallocation();
        this.getqihuomodel();
      })
  ...
  shifangAllocation() {
    ...
    this.xmdqihuoapi.shifangAllocation(model).then(() => {
  ...
  queryallocation() {
    this.xmdqihuoapi.findAllocation(this.qihuoid).then(data => {
  ...
  alldownload() {
    ...
    if (files.length) {
      this.xmdqihuoapi.downlodezhibao({ pathList: files }).then(data => {
        if (data['zipurl']) {
  ...
  /**已下单和已审核状态下可以一键撤回制单中 */
  oneRecall() {
    this.xmdqihuoapi.onerecall(this.qihuoid).then(data => {
      this.getqihuomodel();
      this.findqihuodet();
    });
  }
    ...
    if (this.qihuoflag['qihuochangestatus'] === 0) {
      if (confirm('库存入库前，可变更项 ：价格、买方公司、运输方式、规格、订货量\n下单备注、销售备注、外部交期、内部交货期、钢厂交货地\n库存入库后，可变更项 ：价格、买方公司、运输方式\n点击后合同不能被其他单据引用，确定要变更吗？')) {
        // if (this.qihuoflag['isorderchange']) {
          this.xmdqihuoapi.orderchange(this.qihuoid).then(data => {
            this.getqihuomodel();
            this.findqihuodet();
          });
        // }
  ...
  qhchangetijiao() {
    ...
    if (confirm('确定要提交期货变更吗？')) {
      ...
      this.xmdqihuoapi.orderchangesubmitverify(json).then(data => {
        this.getqihuomodel();
        this.findqihuodet();
  ...
  selectegangchang(gn, chandi) {
    ...
    // this.xmdqihuoapi.getchandigongcha().then(data => {
    //   data.forEach(element => {
  ...
  mdmcreate() {
    if (!this.qihuodetmodel['gcid']) { this.toast.pop('warning', '物料编码不允许为空！'); return; }
    ...
    if (this.qihuodetmodel['detid']) { // 修改明细
      this.xmdqihuoapi.modifymdmqihuodet(this.qihuodetmodel).then(() => {
      ...
      this.xmdqihuoapi.createqihuodet(this.qihuodetmodel).then(() => {
        this.createmdmqihuodialogcoles();
        /* 刷新明细页面*/
        this.findqihuodet();
        this.getqihuomodel();
  ...
  querymat() {
    if (this.qihuodetmodel['matcode'] && this.qihuodetmodel['matcode'].trim()) {
        this.xmdqihuoapi.getmat(this.qihuodetmodel['matcode']).then(mat => {
  ...
  deleteqihuodet() {
    ...
    if (confirm('你确定要删除吗？')) {
      this.xmdqihuoapi.delqihuoDet(this.qihuodetids).then(data => {
      this.toast.pop('success', '删除成功！');
      this.getqihuomodel();
      this.findqihuodet();
      });
  ...
  delfujian(key) {
    ...
    this.xmdqihuoapi.delfujian(params).then(data => {
      this.getfujians();
    });
  }
  uploadfujian(url) {
    const params = {id: this.qihuomodel['id'], url: url};
    this.xmdqihuoapi.uploadfujian(params).then(data => {
      this.getfujians();
    });
  }
  // 上传成功执行的回调方法
  ...
  getfujians(){
    this.xmdqihuoapi.findfujians(this.qihuoid).then(data => {
  ...
  createRelease() {
    ...
    if (confirm('你确定释放货物吗？')) {
      ...
      this.xmdqihuoapi.emancipation(this.params).then(() => {
