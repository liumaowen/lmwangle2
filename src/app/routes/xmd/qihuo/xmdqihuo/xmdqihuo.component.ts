...
@Component({
  selector: 'app-xmdqihuo',
  templateUrl: './xmdqihuo.component.html',
  styleUrls: ['./xmdqihuo.component.scss']
})
export class XmdQihuoComponent implements OnInit {
  ...
  constructor(private qihuoApi: XmdQihuoService, private toast: ToasterService, private router: Router,
  ...
  addopen() {
    ...
    this.sellers = [{ label: '请选择。。。', value: null },{ label: '山东新美达科技材料有限公司', value: 26516 }];
    ...
    if (confirm('期货订单实付定金低于10万或者定金比例小于15%，最后一次释放定金。你确定要创建吗？')) {
      this.qihuoApi.add(this.qihuo).then((data) => {
        ...
        if (this.qihuo['ordertype'] === 2) {
          ...
          this.router.navigate(['xmdqihuo', data]);
        }
      });
    }
