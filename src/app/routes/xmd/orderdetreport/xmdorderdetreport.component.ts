...
@Component({
  selector: 'app-xmdorderdetreport',
  templateUrl: './xmdorderdetreport.component.html',
  styleUrls: ['./xmdorderdetreport.component.scss']
})
export class XmdOrderdetreportComponent implements OnInit {
  // 品名选择弹窗
  ...
  listDetail() {
    ...
    this.xmdapi.orderdet(this.requestparams).then((response) => {
  ...
  openQueryDialog() {
    if (this.sellersResult.length < 1) {
      ...
      this.xmdapi.findwiskind().then((response) => {
    ...
    if (this.cangku.length < 1) {
      ...
      this.xmdapi.cangkulist().then((response) => {
        response.forEach(element => {
          this.cangku.push({
            value: element.id,
            label: element.name
