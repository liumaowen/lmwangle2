...
@Component({
  selector: 'app-xmdkucundetail',
  templateUrl: './xmdkucundetail.component.html',
  styleUrls: ['./xmdkucundetail.component.scss']
})
export class XmdkucundetailComponent implements OnInit {
  ...
  constructor(public settings: SettingsService, private toast: ToasterService, private userapi: UserapiService,
    ...
    private router: Router, private classifyapi: ClassifyApiService,private rukuapi: RukuService,private xmdapi : XmdService) {
  ...
  selectGnAction(key) {
    ...
    this.xmdapi.getConditions({ gn: this.search['gn'] }).then(data => {
      this.data = data;
      this.filter();
      // data.forEach(element => {
