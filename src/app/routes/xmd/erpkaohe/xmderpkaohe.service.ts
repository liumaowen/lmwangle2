...
@Injectable()
export class XmderpkaoheService {
    ...
    find(search) {
        return this.http.get('store/api/xmd/yanqitihuo/find', { search: search }).toPromise().then(data => {
            return data.json() as any[];
        });
    }
    ...
    cancelFundInterest(params) {
        return this.http.post('store/api/xmd/yanqitihuo/cancelfundinterest', params).toPromise().then(data => {
            return data.json() as any[];
        });
    }
}