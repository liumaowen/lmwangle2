...
import {XmdorderService} from '../xmdorder.service';
import { XmdQihuoService } from '../../qihuo/xmdqihuo.service';
import { XmdkucundetimportComponent } from 'app/dnn/shared/xmdkucundetimport/xmdkucundetimport.component';
import { XmdBusinessorderapiService } from '../xmdbusinessorderapi.service';
import { XmdGoodscodeService } from '../../goodscode/xmdgoodscode.service';

const sweetalert = require('sweetalert');

@Component({
  selector: 'app-xmdorderdetail',
  templateUrl: './xmdorderdetail.component.html',
  styleUrls: ['./xmdorderdetail.component.scss']
})
export class XmdorderdetailComponent implements OnInit {
  results: any;
  ...
  constructor(public settings: SettingsService, private route: ActivatedRoute, private router: Router,
              private storage: StorageService, private toast: ToasterService, private bsModal: BsModalService,
              private datepipe: DatePipe, private modalService1: BsModalService, private modalService: BsModalService,
              private classifyapi: ClassifyApiService, private businessorderApi: XmdBusinessorderapiService, private userApi: UserapiService,
              private orderApi: OrderapiService, private moneyapi: MoneyService, private xmdqihuoapi: XmdQihuoService, private rukuApi: RukuService,
              private matchcarApi: MatchcarService, private xmdApi: XmdorderService) {
    this.gridOptions = {
  ...
  vuserRenderer(val: string, regStr: any): any {
    if (regStr === undefined || regStr === null) {
      return val;
    }
    if (!val) {
      return null;
  ...
  /**查询配款明细表 */
  queryallocation() {
    this.xmdApi.findAllocation(this.orderid).then(data => {
  ...
  ckfeetypeonHide() {
    this.ckfeetype.onHide.subscribe(data => {
      if (!isNaN(this.chukufeetypess['value'])) {
        ...
        this.xmdApi.getchukufeetype(this.chukufeetypess).then(() => {
          this.listDetail();
          this.getDetail();
  ...
  // 获取网格中的数据
  listDetail() {
    this.xmdApi.getorderinfo(this.route.params['value']['id']).then((data) => {
  ...
  getDetail() {
    this.xmdApi.getorderdet(this.orderid).then((data) => {
  ...
  importKucun() {
    ...
    this.kcbsModalRef = this.bsModal.show(XmdkucundetimportComponent);
  ...
  submitVerify() {
    ...
    if (this.businessorder['compensation'] === 0) {
      if (confirm('该质量异议钢厂尚未赔付，属于先行赔付，需审批到总经理，是否确认提交')) {
        this.businessorderApi.submitVuser(this.businessorder['id'], {
          vuserid: this.businessorder['vuserid'],
          beizhu: this.businessorder['beizhu']
        }).then((data) => {
          this.businessorder = data['order'];
  ...
  refuseverify() {
    if (confirm('你确定要拒绝审核吗？')) {
      // tslint:disable-next-line:max-line-length
      this.businessorderApi.refuseverify(this.businessorder['id'], {
        msg: '该订单不予审核，请审查无误再次提交审核！',
        version: this.businessorder['version']
      }).then((data) => {
        this.businessorder = data['order'];
        this.router.navigateByUrl('order');
  ...
  payorderDialog() {
    ...
    if (this.b) {
      if (confirm('该订单销售单价＜（=）考核单价，请确认是否需要变更')) {
        ...
        this.payordershow();
        this.businessorderApi.getmoney1({
          buyerid: this.businessorder['buyerid'],
          wcustomerid: this.businessorder['sellerid'],
          salemanid: this.businessorder['salemanid']
        }).then(data => {
          if (!data['wyue']) {
  ...
  arrearspayshow() {
    ...
    this.qiankuantypes = [{value: '1', label: '正常欠款'}, {value: '2', label: '质押金欠款'}, {
      value: '3',
      label: '客户货到付款'
    }, {value: '4', label: '小数点问题'}, {value: '5', label: '其他'}];
  ...
  // 欠款支付
  arrearspay() {
    this.businessorderApi.getmoney({
      buyerid: this.businessorder['buyerid'],
      wcustomerid: this.businessorder['sellerid'],
      salemanid: this.businessorder['salemanid']
    }).then((data) => {
  ...
  // 查看已打印的订单
  print() {
    this.xmdApi.print(this.route.params['value']['id']).then((response) => {
      if (!response['flag']) {
        this.toast.pop('warning', response['msg']);
  ...
  reload() {
    ...
    this.xmdApi.reload(this.route.params['value']['id'], this.changeordermodal).then((response) => {
      // Notify.alert(response.msg, { status: 'warning' });
      this.toast.pop('warning', response['msg']);
    ...
    if (this.flag['qihuochangestatus'] === 0) {
      if (confirm('待付款状态，可变更项 ：考核价格\n点击后合同不能被其他单据引用，确定要变更吗？')) {
        if (this.flag['isorderchange']) {
          this.xmdqihuoapi.orderchange(this.orderid).then(data => {
            this.listDetail();
            this.getDetail();
          });
        }
  ...
  getorderchangeloglist() {
    this.xmdqihuoapi.getqihuochangeloglist(this.orderid).then(res => {
      const data = res['list'];
