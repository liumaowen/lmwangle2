...
@Component({
  selector: 'app-xmdorder',
  templateUrl: './xmdorder.component.html',
  styleUrls: ['./xmdorder.component.scss']
})
export class XmdOrderComponent implements OnInit {
  results: any;
  activating = false;
  ...
  constructor(private storage: StorageService,
              private xmdApi: XmdorderService,
  ...
  listDetail() {
    this.xmdApi.query(this.search).then(data => {
      // 获取到总条目
  ...
  status = [{label: '请选择订单状态', value: ''}, {label: '制单中', value: 0}, {label: '运费核算中', value: 1},
    {label: '待付款', value: 2}, {label: '待提货', value: 3}, {label: '完成', value: 6}, {label: '取消', value: 7}, {
      label: '撤销',
      value: 8
    }];
  ...
  findWiskind() {
    if (this.companyIsWiskind.length < 1) {
      ...
      this.companyIsWiskind.push({ label: '山东新美达科技材料有限公司', value: 26516 });
    }
  }
  ...
  create() {
    ...
    this.xmdApi.create(this.businessorder).then((datas) => {
      // ngDialog.close();
      this.hidecreateModal();
      this.router.navigate(['xmdorderdet', datas['id']]);
