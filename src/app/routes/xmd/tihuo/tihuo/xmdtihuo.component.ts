...
@Component({
  selector: 'app-xmdtihuo',
  templateUrl: './xmdtihuo.component.html',
  styleUrls: ['./xmdtihuo.component.scss']
})
export class XmdTihuoComponent implements OnInit {
  ...
  status = [{label: '请选择订单状态', value: ''}, {label: '已安排发货', value: 1}, {label: '待付款', value: 2}, {
    label: '货物已出库',
    value: 3
  }, {label: '已作废', value: 4}];
  ...
  constructor(private toast: ToasterService, private tihuoApi: XmdtihuoService) {
    this.querydata();
  }
  ...
  modifytihuoren(model) {
    ...
    this.params = {
      siji: model.siji,
      sijiid: model.sijiid,
      sijitel: model.sijitel,
      chehao: model.chehao,
      transcompanyid: model.transcompanyid,
      yunprice: model.yunprice,
      ist: model.ist,
      id: model.id
    };// 得到要修改的司机信息
    if (model.status == 1) {
