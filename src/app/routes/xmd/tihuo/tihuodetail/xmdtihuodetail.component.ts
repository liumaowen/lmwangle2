...
@Component({
  selector: 'app-xmdtihuodetail',
  templateUrl: './xmdtihuodetail.component.html',
  styleUrls: ['./xmdtihuodetail.component.scss']
})
export class XmdTihuodetailComponent implements OnInit {
  ...
  interestAddParams = {
    ...
    qiankuandays: '', qiankuanreason: '', yedate: '', isqiankuan: false, yue: 0, qiankuanhuokuanjine: 0
  }; // 资金占用利息登记对象
  // 评价物流弹窗
  ...
  constructor(public settings: SettingsService, private fb: FormBuilder, private tihuoApi: XmdtihuoService,
              ...
              private storage: StorageService, private toast: ToasterService, private businessOrderApi: XmdBusinessorderapiService,
    ...
    this.ldgridOptions.columnDefs = [
      ...
      },
      {
        ...
        cellRenderer: (params) => {
          if (params.data.tihuo.type === 0) {
            return '自提';
          } else if (params.data.tihuo.type === 1) {
            return '代运';
          }
        }
      },
  ...
  paytihuoDialog() {
    ...
    this.businessOrderApi.getmoney1({
      buyerid: this.tihuo.buyerid,
      wcustomerid: this.tihuo.sellerid,
      salemanid: this.tihuo['salemanid']
    }).then((data) => {// 仅仅是获取当前客户的余额
      if (!data['wyue']) {
  ...
  feetype = [{label: '请选择', value: ''}, {label: '汽运费', value: 1}, {label: '铁运费', value: 2}, {
    label: '船运费',
    value: 3
  }, {label: '出库费', value: 4}, {label: '开平费', value: 5}, {label: '纵剪费', value: 6}, {
    label: '销售运杂费',
    value: 7
  }, {label: '包装费', value: 8}, {label: '仓储费', value: 9}, {label: '保险费', value: 10}, {label: '押车费', value: 11}];
  ...
  showupdatemanyFee() {
    ...
    for (let i = 0; i < feecollectLists.length; i++) {
      if (types !== feecollectLists[i].data.type) {
        typecount++;
        types = feecollectLists[i].data.type;
      }
      if (prices !== feecollectLists[i].data.price) {
        pricecount++;
        prices = feecollectLists[i].data.price;
      }
      if (feenames !== feecollectLists[i].data.feename) {
        feenamecount++;
        feenames = feecollectLists[i].data.feename;
      }
      if (typecount > 1) {
  ...
  // 关闭查询弹窗
  hideInterestAddModal() {
    this.interestAddModal.hide();
  }
