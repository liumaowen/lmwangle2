...
@Component({
  selector: 'app-xmdreceivedetail',
  templateUrl: './xmdreceivedetail.component.html',
  styleUrls: ['./xmdreceivedetail.component.scss']
})
export class XmdreceivedetailComponent implements OnInit {
  ...
  isverify = false; // 审核
  iswuliubu: boolean;
  constructor(private numberpipe: DecimalPipe,private route: ActivatedRoute, private receiveApi: XmdreceiveapiService, private customerApi: CustomerapiService,
  ...
  submitverify() {
    ...
    if (confirm('确定要提交审核吗？')) {
      this.receiveApi.submitverify(this.model['id'], this.model).then((model) => {
        ...
        this.router.navigateByUrl('xmdreceive');
      });
    }
  }
  // 通过审核
