...
@Component({
  selector: 'app-xmdreceive',
  templateUrl: './xmdreceive.component.html',
  styleUrls: ['./xmdreceive.component.scss']
})
export class XmdreceiveComponent implements OnInit {
  ...
  constructor(private receiveApi:XmdreceiveapiService, private orgApi: OrgApiService,
    private datepipe: DatePipe, private xmdcustomerApi: XmdcustomerService,
  ...
  // 启动添加对话框
  createDialog() {
    this.receivecustomer = [{ label: '请选择...', value: null },{ label: '山东新美达科技材料有限公司', value: 26516 }];
    this.companyflag = { type: 'false' };
    // 得到公司所有的收款银行和账号防止输入有误
    this.companyOfName = {};
    this.xmdcustomerApi.findwiskind().then((data) => {
      ...
      if (str) {
        if (confirm('确定创建吗？')) {
          ...
          this.receiveApi.create(this.model).then(model => {
            this.router.navigate(['xmdreceive', model['id']]);
          });
        }
      } else {
