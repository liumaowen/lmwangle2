...
@Component({
  selector: 'app-privatesaledet',
  templateUrl: './privatesaledet.component.html',
  styleUrls: ['./privatesaledet.component.scss']
})
export class PrivatesaledetComponent implements OnInit {
  ...
  // 网格赋值
  listDetail() {
    this.orderApi.getprivatesaledet(this.search).then(data => {
      this.model = data['map'];
  ...
  getorgplanlist() {
    this.orderApi.getprivateplanList().then(data => {
      this.orgplangridOptions.api.setRowData(data);
    });
  }
  getdaigplist() {
    this.orderApi.getprivateDaigpList().then(data => {
  ...
  updatedaigp(params, filed) {
    ...
    this.orderApi.updateprivatedaigp(params.node.data).then(data => {
      this.getdaigplist();
    }, err => {
      params.node.data[filed] = params.oldValue;
      params.node.setData(params.node.data);
  ...
  updateorgplan(params, filed) {
    ...
    this.orderApi.updateprivateplan(params.node.data).then(data => {
      this.getorgplanlist();
    }, err => {
      params.node.data[filed] = params.oldValue;
      params.node.setData(params.node.data);
  ...
  addorg() {
    ...
    this.orderApi.createprivateplan(obj).then(data => {
      this.orgcoles();
      this.getorgplanlist();
    });
  }
  /**根据机构和年份删除 */
  delete(params) {
    this.orderApi.deleteprivateplan({orgid: params.orgid, year: params.year}).then(data => {
      this.getorgplanlist();
    });
  }
  gethistory() {
    this.bakModal.show();
    this.orderApi.privatesalegroupByMonth().then(data => {
