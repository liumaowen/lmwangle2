...
@Component({
  selector: 'app-cgwanglaiyue',
  templateUrl: './cgwanglaiyue.component.html',
  styleUrls: ['./cgwanglaiyue.component.scss']
})
export class CgwanglaiyueComponent implements OnInit {
  ...
  constructor(private caigouApi: CaigouService, public settings: SettingsService, private datepipe: DatePipe,
    ...
    this.gridOptions.columnDefs = [
      ...
      {
        cellStyle: { 'text-align': 'right' }, headerName: '入库金额', field: 'caigoujine', minWidth: 90,
        valueFormatter: this.settings.valueFormatter2
      },
      {
        cellStyle: { 'text-align': 'right' }, headerName: '补差金额', field: 'buchajine', minWidth: 90,
        valueFormatter: this.settings.valueFormatter2
      },
      {
        cellStyle: { 'text-align': 'right' }, headerName: '退货金额', field: 'tuihuojine', minWidth: 90,
        valueFormatter: this.settings.valueFormatter2
      },
      {
        cellStyle: { 'text-align': 'right' }, headerName: '账户余额', field: 'yue', minWidth: 90,
        cellRenderer: (params) => {
          return '<a target="_blank" href="#/cgwanglai/' + params.data.supplierid + 'abc' + params.data.wcustomerid + '">'
            + params.data.yue + '</a>';
        }, valueFormatter: this.settings.valueFormatter2
      },
      {
        cellStyle: { 'text-align': 'right' }, headerName: '品名', field: 'gn', minWidth: 90
      },
      {
        cellStyle: { 'text-align': 'right' }, headerName: '产地', field: 'chandi', minWidth: 90
      },
      { cellStyle: { 'text-align': 'center' }, headerName: '机构', field: 'orgname', minWidth: 90 },
      { cellStyle: { 'text-align': 'center' }, headerName: '是否外贸', field: 'isft', minWidth: 90 },
  ...
  querydata() {
    this.caigouApi.getwanglaiyue(this.search).then(data => {
