...
@Component({
  selector: 'app-zaituruku',
  templateUrl: './zaituruku.component.html',
  styleUrls: ['./zaituruku.component.scss']
})
export class ZaiturukuComponent implements OnInit {
  ...
  // 查询数据
  querydata() {
    this.rukuapi.zaituquery(this.search).then((data) => {
      // 获取到总条目
  ...
  // 查询
  select() {
    this.querys['billno'] ? this.search['billno'] = this.querys['billno'] : '';
  ...
  uploads($event) {
    ...
    if ($event.length !== 0) {
      this.rukuapi.createzaitu(addData).then(data => {
        console.log(1111111);
        console.log(data.grnoList);
        if(data.grnoList !== null && data.grnoList !== undefined){
          if(confirm(data.grnoList+'资源号明细属我司承运，请及时进行物流竞价')){
              // this.singleData.unshift(data);
              this.querydata();
              this.toast.pop('success', '上传成功！');
              this.router.navigate(['zaituruku', data.zaituKucunnew.id]);
              this.querydata();
              this.hideDialog();
          }
        }        
  ...
  /**下载模板 */
  createExcel() {
    this.rukuapi.createExcel().then(data => {
