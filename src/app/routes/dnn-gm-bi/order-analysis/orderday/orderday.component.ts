...
@Component({
  selector: 'app-orderday',
  templateUrl: './orderday.component.html',
  styleUrls: ['./orderday.component.scss'],
...
export class OrderdayComponent implements OnInit {
  ...
  constructor(public settings: SettingsService, private gmbiApi: GmbiService, private userApi: UserapiService, private datepipe: DatePipe, private numpipe: DecimalPipe, private toasterService: ToasterService) {
    ...
    this.gridOptions.columnDefs = [
      ...
      { cellStyle: { "text-align": "right" }, headerName: '库存', field: 'kucun', minWidth: 70, aggFunc: 'sum', valueFormatter: this.settings.valueFormatter3 },

      {
        cellStyle: { "display": "block" }, headerName: '未产', headerClass: 'wis-ag-center',
        children: [
          { cellStyle: { "text-align": "right" }, headerClass: 'text-red', headerName: '未产', field: 'weichan', minWidth: 60, aggFunc: 'sum', editable: true, valueFormatter: this.settings.valueFormatter3, newValueHandler: this.newValueHandler },
          { cellStyle: { "text-align": "right" }, headerClass: 'text-red', headerName: '预定', field: 'yuding', minWidth: 60, aggFunc: 'sum', editable: true, valueFormatter: this.settings.valueFormatter3, newValueHandler: this.newValueHandler },
        ]
      },

      { cellStyle: { "text-align": "right" }, headerName: '现货总量', field: 'xianhuo', minWidth: 90, aggFunc: 'sum', valueFormatter: this.settings.valueFormatter3, valueGetter: (params) => params.data['kucun'] + params.data['weichan'] - params.data['yuding'] },
      ...
      { cellStyle: { "text-align": "right" }, headerName: '90天以上库存', field: 'kucun90', minWidth: 120, aggFunc: 'sum', valueFormatter: this.settings.valueFormatter3 },
  ...
  newValueHandler = (params) => {
    //要查询条件
    if (params.data.riqi != this.datepipe.transform(new Date(), "y-MM-dd") || params.newValue == params.oldValue) return false;
    let num = new RegExp("^[0-9]*$");
    if (num.test(params.newValue)) {
      let search = { chandiid: '', gnid: '', areaid: '' };
      search.chandiid = params.data["chandiid"];
      search.gnid = params.data["gnid"];
      search.areaid = params.data["areaid"];
      search[params.colDef["field"]] = params.newValue;
      this.gmbiApi.editweichan(search).then((data) => {
        params.data[params.colDef.field] = Number(params.newValue);
        params.api.recomputeAggregates()
        // return true
      }).catch(da => {
        search[params.colDef["field"]] = params.oldValue;
      ...
      this.toasterService.pop("error", "格式不正确，请填写整数");
      return false
    }
  }
  ...
  querylist() {
    ...
    this.gmbiApi.getOfday(this.search).then(data => {
