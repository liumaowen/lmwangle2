...
@Component({
    ...
    styleUrls: ['./usermonth.component.scss']
})
export class UsermonthComponent implements OnInit {
    ...
    constructor(public settings: SettingsService, private gmbiApi: GmbiService, private datepipe: DatePipe, private numpipe: DecimalPipe) {
        ...
        this.gridOptions.columnDefs = [
            { headerName: '月份', field: 'nianyue', minWidth: 95, cellRenderer: 'group', showRowGroup: 'nianyue', rowGroup: true },
            ...
            //   children: [
            //     {cellStyle: { "text-align": "right" }, headerName: '买入', field: 'insidebuy', minWidth: 70,aggFunc: 'sum',valueFormatter: this.settings.valueFormatter},
            //     {cellStyle: { "text-align": "right" }, headerName: '卖出', field: 'insidesale', minWidth: 80,aggFunc: 'sum',valueFormatter: this.settings.valueFormatter}
            // ]},

            { cellClass: 'text-right', headerName: '内采', field: 'inside', minWidth: 60, aggFunc: 'sum', valueFormatter: this.settings.valueFormatter2 },

            { cellClass: 'text-right', headerName: '退货', field: 'tuihuo', minWidth: 60, aggFunc: 'sum', valueFormatter: this.settings.valueFormatter2 }
    ...
    querylist() {
        ...
        if (this.search.startdate && this.search.enddate) {
            search.start = this.datepipe.transform(this.search.startdate, 'y-MM');
            search.end = this.datepipe.transform(this.search.enddate, 'y-MM');
        }
        this.gmbiApi.getOfusermonth(search).then(data => {
