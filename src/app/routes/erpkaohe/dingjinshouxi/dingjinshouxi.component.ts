...
@Component({
  selector: 'app-dingjinshouxi',
  templateUrl: './dingjinshouxi.component.html',
  styleUrls: ['./dingjinshouxi.component.scss']
})
export class DingjinshouxiComponent implements OnInit {
  search: any = {};
  ...
  listDetail() {
    ...
    // 从服务器获取数据赋值给网格
    this.erpkaoheapi.dingjinshouxi(this.search).then((data) => {
  ...
  calcdingjinshouxi() {
    if (confirm('你确定计算吗？')) {
      this.erpkaoheapi.calcdingjinshouxi(this.search).then((data) => {
      });
    }
  }
