...
import { XmdService } from 'app/routes/xmd/xmd.service';

@Component({
  selector: 'app-xmdkucundetimport',
  templateUrl: './xmdkucundetimport.component.html',
  styleUrls: ['./xmdkucundetimport.component.scss']
})
export class XmdkucundetimportComponent implements OnInit {
  ...
  constructor(public bsModalRef: BsModalRef, public settings: SettingsService, private kucunapi: KucunService,
    ...
    private tihuoApi: XiaoshouapiService, private businessOrderApi: XmdBusinessorderapiService, private customerApi: CustomerapiService,
    private proOrderApi: ProorderapiService,private xmdapi : XmdService) {
    ...
    //   onCellValueChanged: (params) => { this.kucunapi.modifyBeizhu({ kucunid: params.data.kucunid, beizhu: params.data.beizhu }); },
      ...
      onGridReady: () => {
        if (this.storage.getObject('permanent_xmdkucunTablePosition')) {
          this.gridOptions.columnApi.setColumnState(this.storage.getObject('permanent_xmdkucunTablePosition'));
        }
      }
    };
  ...
  // 查询库存明细表
  listDetail() {
    this.xmdapi.listDetail(this.search).then(data => {
      this.msg = '';
  ...
  selectGnAction(key) {
    ...
    this.xmdapi.getConditions({ gnid: this.search['gnid'] }).then(data => {
      this.data = data;
      this.filter();
    });
    this.disabled = false;
